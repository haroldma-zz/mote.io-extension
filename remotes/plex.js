    ...
    data: [{
      press: function() {
        if (mIndex > 0) {
          ...
          $($('.tile-list li').find('a .poster-container, .section-poster')[mIndex]).css({
            'outline': ''
          })
          mIndex--;
        }
        $($('.tile-list li').find('a .poster-container, .section-poster')[mIndex]).css({
          'outline': '5px solid #ff9d00'
        })
  ...
  mote.io.remote = {
    ...
    update: function(force) {
      ...
      if (pflag) {
        ...
        mote.io.notify(
        $($('.video-details h2').find('a')[0]).text(), $($('.video-details h2').find('a')[1]).text(), 'http://www.appscout.com/images/Plex-iOS-Logo.jpg', window.location.href, force);
      }
    },
    blocks: navBlocks
  };
